...
router.get('/:title', async (req, res) => {
  try {
    ...
    const url = `https://streaming-availability.p.rapidapi.com/search/title?title=${req.params.title}&country=us&show_type=all&output_language=en`;
    const options = {
      method: 'GET',
      headers: {
    ...
    console.log(movies.result[0]);
    if (movies) 
{ res.render("results", movies.result[0]) }
